    ...
    server.tool(
        ...
        async ({ parameters }) => {
            try {
                ...
                const frameworkPath = path.join(STYLES_DIR, "mcp-code.md");
                const frameworkContent = await loadContentOrDefault(
                    frameworkPath,
                    "# Framework MCP-CODE\n\nEstructura de proyecto dividida en code/ (implementación) y mcp/ (gestión)."
                );
                ...
                const modoDesc = await loadContentOrDefault(
                    profilePath,
                    `Actuaré como un ${modo} con experiencia en desarrollo de software.`
                );

                // Cargar la descripción del proyecto
                let projectType = proyecto.toLowerCase();
                // Normalizar algunos tipos comunes
                if (["ml", "ia", "machine-learning", "inteligencia-artificial"].includes(projectType)) {
                    projectType = "ai";
                } else if (["backend", "rest", "graphql", "microservices"].includes(projectType)) {
                    projectType = "api";
                } else if (["frontend", "spa", "client", "ui"].includes(projectType)) {
                    projectType = "web";
                }
                ...
                const proyectoDesc = await loadContentOrDefault(
                    projectPath,
                    `Este proyecto es de tipo ${proyecto}. Se aplicarán principios generales de desarrollo de software.`
                );
                ...
                try {
                    ...
                    if (basic && basic.c && basic.c.length > 0) {
                        projectContext = "He analizado la estructura actual del proyecto. ";
                    }
                } catch (e) {
