...
import { BasicNode, DetailedElement, DetailedNode, IGNORE_DIRS } from '../types/index.js';
// import glob from 'glob';

/**
 * Escanea un proyecto y genera árboles de directorios
 */
export async function scanProject(rootDir: string): Promise<{ basic: BasicNode, detailed: DetailedNode }> {
    // Crear nodo raíz para el árbol básico
    const basicRoot: BasicNode = {
    ...
    await scanDirectory(rootDir, basicRoot, detailedRoot);

    return { basic: basicRoot, detailed: detailedRoot };
}
    ...
    try {
        ...
        for (const item of items) {
            ...
            if (stats.isDirectory()) {
                ...
                // Añadir a los padres
                basicParent.c = basicParent.c || [];
                basicParent.c.push(basicNode);

                detailedParent.c = detailedParent.c || [];
                detailedParent.c.push(detailedNode);

                // Escanear recursivamente
                await scanDirectory(itemPath, basicNode, detailedNode);
            } else {
                // Es un archivo
                const basicNode: BasicNode = {
                    n: item,
